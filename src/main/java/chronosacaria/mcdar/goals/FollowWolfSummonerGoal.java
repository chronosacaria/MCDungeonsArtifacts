...
public class FollowWolfSummonerGoal extends Goal {
    ...
    private boolean canTeleportTo(BlockPos blockPos) {
        if (LandPathNodeMaker.getLandNodeType(tastyBoneWolfEntity.getEntityWorld(), new BlockPos.Mutable()) != PathNodeType.WALKABLE || !this.leavesAllowed && this.worldView.getBlockState(blockPos.down()).getBlock() instanceof LeavesBlock)
            return false;
        return this.worldView.isSpaceEmpty(this.tastyBoneWolfEntity, this.tastyBoneWolfEntity.getBoundingBox().offset(blockPos.subtract(new BlockPos(this.tastyBoneWolfEntity.getBlockPos()))));
    }
